...
def cross_entropy(pred,
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            # NOTE: label may contain ignore_index (e.g., 255). We must NOT index

            # class_weight by ignore_index, otherwise it will be out-of-bounds.

            label_flat = label.reshape(-1).long()

            valid = (label_flat != ignore_index)

            label_safe = label_flat.clone()

            label_safe[~valid] = 0  # safe index for gather

            # class_weight is already a Tensor on correct device in this file

            label_weights = class_weight.gather(0, label_safe)

            # If avg over non-ignore, ignore pixels contribute 0 weight.

            # Even if avg_non_ignore=False, setting ignore to 0 here avoids OOB.

            label_weights = label_weights * valid.float()
